...
func UserRouter(rf *gin.RouterGroup) {
	ru := rf.Group("/user")
	{
		...
		ru.GET("/devices/userId/:userId", user.Devices) //获取所有的设备号
		...
		rus.GET("/group/groupId/:groupId/members", user.GroupMembers) //查询群所有成员
		rus.GET("/groups/userId/:userId", user.UserGroups)            //某人有多少群
	}
}
